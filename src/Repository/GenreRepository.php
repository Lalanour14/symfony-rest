...
class GenreRepository
{
     ...
     */
    public function findByMovie(int $id): ?Genre
    {

        $connection = Database::getConnection();

        $query = $connection->prepare("SELECT * FROM genre LEFT JOIN genre_movie ON genre.id=genre_id.id_genre  WHERE genre_movie.id=:id ");
